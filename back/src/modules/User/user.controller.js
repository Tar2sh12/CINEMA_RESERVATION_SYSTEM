...
export const login = async (req, res, next) => {
  ...
  const token = jwt.sign({ userId: user._id ,role:user.userType, unique_name: user.userName}, process.env.LOGIN_SECRET, {
    expiresIn: "1d",
  });

  // response
  res.status(200).json({ message: "Login success", token , data:user });
};
